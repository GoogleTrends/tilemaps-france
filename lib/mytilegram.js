  ...
  var tooltip = d3.select('body')
    ...
    function ready(error, tilegram, results) {
      ...
      results = results.reduce(function (a, b) {
        //console.log(b)
        if(legend[b.region]){  
          legend[b.region] =  legend[b.region] + 1;
        }else{
          legend[b.region] = 1 ;
        }
        //parses elements in result.csv availaible in your dataset
        a[b.geoId] = { 
          party: b.party,
          region : b.region,
        }
      ...
      $.each(sortableLegend, function(i)
      {
          var li = $('<li/>')
              ...
              .appendTo(cList);
          var aaa = $('<div/>')
              .addClass('party-'+sortableLegend[i][0].split(' ').join('-'))
              .text(sortableLegend[i][0])
              .appendTo(li);
      });
  ...
  return ('party-' + results[d.id].region).split(' ').join('-')
   })
    .on('mouseover', function(d) {
      var ev = d3.event
      tooltip.text(  d.properties.name + ' : ' + results[d.id].party + '/100 d\'intensité des recherches' )
        .style('opacity', 1)
